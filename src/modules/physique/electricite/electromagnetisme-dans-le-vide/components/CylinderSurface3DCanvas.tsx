...
function AmpereContour({ radius, dir, isMain = false, yOffset = 0, isZero = false }: { radius: number, dir: number, isMain?: boolean, yOffset?: number, isZero?: boolean }) {
...
function SurfaceCurrentDensity({ radius, height, dirI }: { radius: number, height: number, dirI: number }) {
  // Generate points ONLY on the surface
...
function CylinderSurfaceScene({ rho, dirI, planeMode, R }: { rho: number, dirI: number, planeMode: string, R: number }) {
  const isInside = rho < R;
  // if rho < R : B = 0
  // if rho > R : B = (mu0 * js * R) / rho
  const bMag = isInside ? 0 : (R * 2) / rho; 
  ...
  return (
    <group position={[0, 0, 0]}>
      ...
      <Html position={[R + 0.5, dirI > 0 ? 3 : -3, 0]} center>
        <div className="text-amber-400 font-bold font-serif italic text-sm drop-shadow-md pointer-events-none">j<sub>s</sub></div>
      </Html>
      ...
      <group position={[rho, 0, 0]}>
        ...
        {/* Magnetic Field Vector B (only if outside) */}
        {!isInside && (
          <group>
...
export default function CylinderSurface3DCanvas() {
        ...
        <div className="w-full mt-2 bg-slate-900/50 rounded-xl border border-slate-800 p-3 sm:p-4 flex items-center justify-center">
          <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm sm:text-base">
            <span className="text-emerald-400 font-bold flex items-center gap-2">
              ...
              {rho < R ? (
                 <LatexMath math={`\\vec{0}`} block={false} />
              ) : (
                 <LatexMath math={`\\frac{\\mu_0 j_s R}{\\rho} \\, \\vec{e_\\theta}`} block={false} />
              )}
            </span>
